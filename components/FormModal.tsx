...
const FormModal: React.FC<FormModalProps> = ({ isOpen ,onSubmit, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-90 z-50">
      <div className="bg-white dark:bg-gray-700 p-8 rounded-lg shadow-lg w-full max-w-lg">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Add Note</h2>
          <button
            onClick={onClose}
        ...
        <form className="space-y-4">
          ...
          <div>
            ...
            <input
              type="file"
              onChange={(e) => setUserPicture(e.target.files ? e.target.files[0].name : '')}
              className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
